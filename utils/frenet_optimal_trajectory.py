...
from utils.quintic_polynomials_planner import QuinticPolynomial
import utils.cubic_spline_planner as cubic_spline_planner

SIM_LOOP = 500

# Parameter
MAX_SPEED = 40#40 # maximum speed [m/s] ## it was 100/3.6
MAX_ACCEL = 8.0#8.0  # maximum acceleration [m/ss]
MAX_CURVATURE = 3.0#3.0  # maximum curvature [1/m]
...
def main():
    ...
    for i in range(SIM_LOOP):
        ...
        if show_animation:  # pragma: no cover
            ...
            plt.plot(tx, ty,color='brown') 
            plt.plot(ob[:, 0], ob[:, 1], "xk")
            plt.plot(path.x[1:], path.y[1:], "x",color='green')
